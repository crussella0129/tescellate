...
- **Sprint Goal:** `.tscl` persistence end-to-end in the egui/wasm UI (Ctrl+S save, Ctrl+O open, localStorage autosave; UiState round-trips).
  - **Component A — Store layer extension** (UiState payload in the zip; format-version bump v0→v1; backwards-compatible read of v0).
    - T-001: Add `UiState` opaque-JSON type to `tescellate-store`.
    ...
    - T-003: Round-trip test in `tescellate-store` for workbook + non-empty UiState; version-tolerance test (v0 read yields empty UiState).
  ...
  - **Component C — UI capture/restore**
    ...
    - T-006: Create `apps/tescellate-ui/src/state_io.rs` with `UiSnapshot` struct (the typed mirror of the store's opaque UiState) and `capture(&app) -> UiSnapshot` / `restore(&mut app, UiSnapshot)` pure functions. Map `UiSnapshot` ↔ `tescellate_store::UiState` via `serde_json`.
  - **Component D — Save/Open keymap and dialog flow**
    - T-007: Add `tescellate-store` dep + `rfd` dep + `base64` dep + `wasm-bindgen-futures` (already present) to `apps/tescellate-ui/Cargo.toml`. Pin versions, configure features so wasm32 compiles.
    ...
    - T-009: Implement async save flow in `app.rs`: on Save, call `engine.save_bytes(&capture())`, hand bytes to `rfd::AsyncFileDialog::new().set_file_name("tescellate.tscl").save_file().await?.write(bytes).await`. Use `wasm-bindgen-futures::spawn_local` on wasm and a `pollster::block_on` (or sync `rfd::FileDialog`) path on native.
    - T-010: Implement async open flow: on Open, `rfd::AsyncFileDialog::new().add_filter("Tescellate", &["tscll", "tscl"]).pick_file().await?.read().await`, then `engine.open_bytes` + `state_io::restore`. Stash bytes in a `Arc<Mutex<Option<Vec<u8>>>>` and drain it at the start of each frame (egui can't await mid-update).
  ...
  - **Component E — localStorage autosave + rehydrate**
    - T-012: `state_io::autosave_to_local_storage(bytes)` — base64 encode, `web_sys::window().local_storage()`, key `tescellate.autosave.v1`. Skip silently if `>4 MiB` (toast hook reserved for later).
    - T-013: `state_io::load_from_local_storage() -> Option<Vec<u8>>` for boot path; in `TescellateApp::new`, prefer the autosave over the seed demos when present.
  ...
  - **Component F — Verification + ship**
    - T-015: `cargo fmt --all`, `cargo clippy --all-targets -- -D warnings` on both workspaces (root + apps/tescellate-ui), `cargo test -p tescellate-store -p tescellate-formula`, `cargo build --target wasm32-unknown-unknown --manifest-path apps/tescellate-ui/Cargo.toml`. All four green.
    - T-016: Open PR `webui-v144-tscl-persistence` with the four-check expectation called out.

## Execution Sequence

### T-001: Add `UiState` opaque-JSON type to `tescellate-store`.
- **Touches:** `crates/tescellate-store/src/lib.rs`
- **Depends on:** (none)
...
### T-003: Round-trip + version-tolerance tests in `tescellate-store`.
- **Touches:** `crates/tescellate-store/src/lib.rs` (tests module)
- **Depends on:** T-002
- **Success criterion:** Two new `#[test]` functions: `round_trip_preserves_ui_state` and `reads_v0_as_empty_ui_state`. Both pass under `cargo test -p tescellate-store`.
...
- **Touches:** `crates/tescellate-formula/src/engine.rs`
- **Depends on:** T-002
...
- **Touches:** `apps/tescellate-ui/src/widget.rs`, `apps/tescellate-ui/src/format.rs`, `apps/tescellate-ui/src/conditional.rs`, `apps/tescellate-ui/src/note.rs`
...
- **Notes:** Add `serde = { version = "1", features = ["derive"] }` and `serde_json = "1"` to `apps/tescellate-ui/Cargo.toml`. Apply `#[serde(default)]` field-wise so missing fields tolerate forward drift.

### T-006: `state_io.rs` capture/restore.
- **Touches:** `apps/tescellate-ui/src/state_io.rs` (new), `apps/tescellate-ui/src/lib.rs` (module decl)
- **Depends on:** T-005
- **Success criterion:** `UiSnapshot { active_sheet, stage_mode, square_format/widgets/notes, hex_format/widgets/notes, triangle_format/widgets/notes, conditional_rules, is_fresh_seed: bool }` exists; `capture(&app) -> UiSnapshot` and `restore(&mut app, UiSnapshot)` round-trip every field. Snapshot ↔ `tescellate_store::UiState` is a one-line `serde_json::to_value`/`from_value` each direction.
...
### T-007: Dependencies in `apps/tescellate-ui/Cargo.toml`.
- **Touches:** `apps/tescellate-ui/Cargo.toml`
- **Depends on:** (none)
- **Success criterion:** `cargo build --manifest-path apps/tescellate-ui/Cargo.toml` and `cargo build --target wasm32-unknown-unknown --manifest-path apps/tescellate-ui/Cargo.toml` succeed.
- **Notes:** Pins: `tescellate-store = { path = "../../crates/tescellate-store" }`, `rfd = { version = "0.14", default-features = false }` (let the default backend ship; wasm picks the right one automatically), `base64 = "0.22"`, `serde = "1"` with derive, `serde_json = "1"`. Verify `cargo tree` on wasm32 has no `libz-sys`/`bzip2-sys`.

### T-008: Keymap commands + bindings.
- **Touches:** `apps/tescellate-ui/src/keymap.rs`, `apps/tescellate-ui/src/app.rs` (NAV_KEYS list)
- **Depends on:** (none)
...
- **Touches:** `apps/tescellate-ui/src/ribbon.rs`
- **Depends on:** T-008
...
- **Touches:** `apps/tescellate-ui/src/state_io.rs`
- **Depends on:** T-006, T-007
- **Success criterion:** `pub fn autosave_to_local_storage(bytes: &[u8])` exists. On wasm, base64-encodes and sets `tescellate.autosave.v1`; on native, no-op. Returns `()`; failures are swallowed (autosave is best-effort). Skip when `bytes.len() > 4 * 1024 * 1024`.
- **Notes:** Use `base64::engine::general_purpose::STANDARD`.

### T-013: localStorage rehydrate on boot.
- **Touches:** `apps/tescellate-ui/src/state_io.rs`, `apps/tescellate-ui/src/app.rs` (`TescellateApp::new`)
- **Depends on:** T-012
...
- **Success criterion:** All of `cargo fmt --all --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test --workspace`, plus the equivalents under `apps/tescellate-ui` (`cargo fmt`, `clippy`, `test`, `build --target wasm32-unknown-unknown`) — all exit zero.
