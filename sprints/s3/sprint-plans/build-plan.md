...
- **Touches:** `crates/carbide-formula/src/excellite/lex.rs`
- **Depends on:** (none)
...
### T-302: XLOOKUP.
- **Touches:** `crates/carbide-formula/src/excellite/funcs/lookup.rs`
- **Depends on:** T-301
...
- **Touches:** `crates/carbide-formula/src/excellite/funcs/stats.rs`
- **Depends on:** T-301
...
- **Touches:** `crates/carbide-formula/tests/reference_examples.rs`
- **Depends on:** T-302, T-303
- **Success criterion:** Eleven new `#[test]` functions (six XLOOKUP, five alias) pass under `cargo test -p carbide-formula`. Existing tests stay green.
...
- **Notes:** Engine-only sprint — no wasm build delta. Still verify `cargo build --target wasm32-unknown-unknown --manifest-path apps/carbide-ui/Cargo.toml` for completeness.
