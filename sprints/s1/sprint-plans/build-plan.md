...
- **Sprint Goal:** v145 — wire `.tscl` Save/Open dialogs + localStorage autosave on top of v144's persistence infrastructure.
  - **Component A — Dialog dependencies + keymap**
    - T-101a: Add `rfd`, `base64`, `web-sys` deps to `apps/tescellate-ui/Cargo.toml`.
  ...
  - **Component B — Save / Open flow**
    - T-101d: `pending_open_bytes: Arc<Mutex<Option<Vec<u8>>>>` field on `TescellateApp`; drain at top of `update()`; route through `open_bytes` + `restore_state`.
  ...
  - **Component C — localStorage autosave**
    ...
    - T-102b: `state_io::load_from_local_storage() -> Option<Vec<u8>>`; rehydrate-on-boot in `TescellateApp::new` (skip seed demos when present).
...
### T-101a: Dialog deps in `apps/tescellate-ui/Cargo.toml`.
- **Touches:** `apps/tescellate-ui/Cargo.toml`
- **Depends on:** (none)
- **Success criterion:** Adds `rfd = { version = "0.14", default-features = false }`, `base64 = "0.22"`, and a `[target.'cfg(target_arch = "wasm32")'.dependencies]` block for `web-sys` with features `["Window","Storage","HtmlAnchorElement","Blob","Url","Document"]` + `js-sys`. `cargo build --manifest-path apps/tescellate-ui/Cargo.toml` and `cargo build --target wasm32-unknown-unknown --manifest-path apps/tescellate-ui/Cargo.toml` both compile.
...
- **Depends on:** T-101a
- **Success criterion:** `TescellateApp` gains `pending_open_bytes: Arc<Mutex<Option<Vec<u8>>>>`. At the very top of `update()`, drain the slot: if `Some(bytes)`, call `engine.open_bytes(&bytes)` + `restore_state(ui_state_to_snapshot(&ui))`. Errors are logged via `tracing::warn!` or `eprintln!`, never panic.
...
- **Depends on:** T-101a
- **Success criterion:** `pub fn autosave_to_local_storage(bytes: &[u8])` exists. wasm32-gated: base64-encodes, calls `window.local_storage()?.set_item("tescellate.autosave.v1", &encoded)`, swallows failure. Native: no-op. Skips silently when `bytes.len() > 4 * 1024 * 1024`.
...
- **Depends on:** T-102a
- **Success criterion:** `pub fn load_from_local_storage() -> Option<Vec<u8>>` exists (wasm32: reads the key, base64-decodes; otherwise `None`). `TescellateApp::new`: if `Some(bytes)`, call `engine.open_bytes(&bytes)` + `restore_state` on the resulting `UiSnapshot`; only seed the demo cells when no autosave was found.
...
- **Success criterion:** `cargo fmt --all --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, plus the same on `apps/tescellate-ui`, plus `cargo build --target wasm32-unknown-unknown --manifest-path apps/tescellate-ui/Cargo.toml` — all green.
