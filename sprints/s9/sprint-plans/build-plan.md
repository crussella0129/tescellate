...
- **Sprint Goal:** v152 — Voronoi widgets (close four-lattice symmetry) + Carbide label fix.
  ...
  - **Component B — Voronoi widget field + persistence**
    - T-902: `voronoi_widgets: Widgets<VoronoiCoord>` on `CarbideApp` + `UiSnapshot`; capture/restore.
  - **Component C — Render**
...
### T-901: Carbide label.
- **Touches:** `apps/carbide-ui/src/app.rs` (`engine_label`).
- **Depends on:** (none).
...
- **Depends on:** (none).
- **Success criterion:** `voronoi_widgets: Widgets<VoronoiCoord>` on `CarbideApp` (init `Widgets::default()` + demo seed in T-904). `UiSnapshot.voronoi_widgets` with `#[serde(default)]`. `capture_state` clones it out; `restore_state` calls `replace_with`. Build clean.

### T-903: Voronoi render widget dispatch.
- **Touches:** `apps/carbide-ui/src/app.rs` (`draw_voronoi_grid`).
- **Depends on:** T-902.
...
- **Depends on:** T-902.
- **Success criterion:** `widgets_generic_with_voronoi_coord_round_trip` (Button + Toggle round-trip through JSON). `snapshot_round_trips_through_ui_state` fixture extended to populate `voronoi_widgets`. `cargo test -p carbide-ui` green.
